...
char* get_android_id(uintptr_t java_vm, uintptr_t jni_env, jobject ctx) {
	JavaVM* vm = (JavaVM*)java_vm;
	JNIEnv* env = (JNIEnv*)jni_env;

	jclass cls = (*env)->GetObjectClass(env, ctx);
	jmethodID nJmethodID = (*env)->GetMethodID(env, cls, "getAndroidId", "()Ljava/lang/String;");
...
func GetAndroidId() string {
	...
	err := mobileinit.RunOnJVM(func(vm, env, ctx uintptr) error {
		result = C.GoString(C.get_android_id(C.uintptr_t(vm), C.uintptr_t(env), C.jobject(ctx)))
		return nil
	})
